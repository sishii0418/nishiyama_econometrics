...
Fig_1_longshortspread %>%
    mutate(year = ym(year)) %>%
    ggplot(aes(x = year)) +
    geom_line(aes(y = `短期金利（コールレート）`, color = "短期金利（コールレート）")) +
    geom_line(aes(y = `長期金利（10年物国債利回り）`, color = "長期金利（10年物国債利回り）")) +
    geom_line(aes(y = `長短金利スプレッド`, color = "長短金利スプレッド")) +
    labs(x = "年", y = element_blank(), color = element_blank()) +
...
## 実証例12.4 金利の期間構造と共和分 {#ex12-4 .unnumbered}
`urca::ur.df()`関数でADF検定を行うことができるが, 表12-1に示されたエングル=グレンジャー検定統計量ではなく通常のADF検定の臨界値が出力されるので直接有意性を確認する必要がある.
```{r}
...
ur.df(model_ex12.4$residuals, type = "none", lags = 10, selectlags = "BIC") %>% summary()
```

## 実証例12.5 南極のペンギンの数と日本の GDP （再考） {-}
[実証例12.4]{#ex12-4}同様, 単位根検定の$t$統計量を直接エングル=グレンジャー検定統計量の臨界値と比較する.
...
summary(model_ex12.7)$adj.r.squared
```

## 実証例12.8 金利の期間構造と動学OLS推定 {-}
ヨハンセン検定は`urca::ca.jo()`関数により実装されている.

```{r}
ca.jo(data_ex12.1, type = "trace", ecdet = "none", K = 2) %>% summary()
```
`ecdet = "none"`と設定することで共和分に定数項・トレンド項がないモデルを指定できるはずだが, 実際はトレンド項があるモデルが推定された.

## 実証例12.10 沖縄返還と通貨切替え {-}
<!-- FIXME: 正しく分析できているか確認? -->
```{r}
OkiDat2 <- read_excel("data/12_第12章/OkiDat2.xls", sheet = "okidat2")
data_ex12.10 <- OkiDat2 %>%
    dplyr::select(-...1)
model_ex12.10 <- VAR(data_ex12.10) %>% BQ()
fevd(model_ex12.10, n.ahead = 20)$dlyratio

# 通貨切替え以前 (1956-1970)
data_ex12.10_pre <- OkiDat2 %>%
    filter(...1 <= 1970) %>%
    dplyr::select(-...1)
model_ex12.10_pre <- VAR(data_ex12.10_pre) %>% BQ()
fevd(model_ex12.10_pre, n.ahead = 20)$dlyratio
```
