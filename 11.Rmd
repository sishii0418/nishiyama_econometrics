...
ここでは, [FRED, St. Louis Fed](https://fred.stlouisfed.org/)からデータを取得することにする.
Rでは`fredr`というパッケージによるAPI実装があり, これを使うと簡単にデータをダウンロードすることができる.
API利用には事前に登録とAPIキーの取得が必要で, 取得したAPIキーは`fred_APIkey.txt`に保存しておく.

```{r collapse=T, message=FALSE}
# APIキーを読み込む
fred_APIkey <- scan("fred_APIkey.txt", what = "character")
# 読み込んだAPIキーをセット
...
DEXJPUS <- fredr(
  series_id = "DEXJPUS",
  observation_start = as.Date("1991-04-01"),
...
```{r collapse=T, message=FALSE}
data_ex11.2 <- DEXJPUS %>%
    mutate(S = log(value) * 100) %>%
    mutate(trend = 1:dim(DEXJPUS)[1])
